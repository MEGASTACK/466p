function postSamples = loopyMetropolisRobust(data,A,priorHyperParams,numSamples,saveFile,propStd,varargin)
...
if( nargin==6 )              % no partial results or initial values
	...
	samples = zeros(numParams,NUM_CHAINS,maxSamplesStored);
   	...
   	fprintf(1,'loopyMetropolis: check for overdispersed initial values..\n');
	iter = 1;
	numRetSamples = 1;
...
for chain = 1:NUM_CHAINS
    ...
    [E_xi,E_xi_xj,BPiter] = fastBPbin(currModel.w(:,:,chain),currModel.b(:,chain));
    logOldP(chain) = logOldP(chain) + (N * Fbethe2(currModel.w(:,:,chain),currModel.b(:,chain),E_xi_xj,E_xi));
end;
...
while(1)
    ...
    if( mod(iter,5000)==0 )
        fprintf(1,'Metropolis ITER = %d BPiter=%d\n',iter,BPiter);
    ...
    for chain = 1:NUM_CHAINS
        ...
        [E_xi,E_xi_xj,BPiter] = fastBPbin(potModel.w(:,:,chain),potModel.b(:,chain));
        logNewP(chain) = logNewP(chain) + (N * Fbethe2(potModel.w(:,:,chain),potModel.b(:,chain),E_xi_xj,E_xi));
    end;   
...
if( iter<minIter )
    fprintf(1,'loopyMetropolis: MAX_ITER reached. samples less than requested\n');
end;
if( ~converged )
    fprintf(1,'loopyMetropolis: MAX_ITER reached. chains not converged \n');
end;
if( maxLagAcorr>ACORR_EPS )
    fprintf(1,'loopyMetropolis: MAX_LAG reached at max acorr=%0.2f. possible insufficient thinning\n',maxLagAcorr);
end;


acceptRate = acceptNum/iter;
fprintf(1,'loopyMetropolis: total iter=%d mean acceptRate=%0.2f thinIter=%d\n',iter,mean(acceptRate),thinIter);
if( CONV_SAVE_DIAG )
    save convDiagV6.mat convDiag acceptRate -V6;
end;
